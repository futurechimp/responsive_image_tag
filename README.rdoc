...
There's a generator which copies a JavaScript file into place. For Rails this will be <tt>public/javascripts</tt> but in Padrino you can specify an alternative location.
...
In order for the JavaScript image replacement to happen, you need to put the following mehtod call in a `$(document).ready()` block or where ever you are .
